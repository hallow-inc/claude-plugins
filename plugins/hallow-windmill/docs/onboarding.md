...
- **Prefer folder ACLs over new Windmill groups.** (The old CE 3-group cap is GONE — Hallow's customized-OSS fork neutered it via `deviation: neuter CE group cap`, so group creation no longer hard-fails at 3. But folder ACLs remain the Hallow convention; don't proliferate groups just because you now can. Full decision guide: `docs/folders-groups.md` §0. Ask an admin if a new group is genuinely warranted.)

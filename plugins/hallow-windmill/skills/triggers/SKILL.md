...
**Hallow ban:** `wmill sync push` and `wmill sync pull` are banned in this workspace. They delete server state not in local files and clobber secret variables. Mirror trigger changes to the server via the MCP `windmill` tools or the Windmill UI — never `wmill sync`.

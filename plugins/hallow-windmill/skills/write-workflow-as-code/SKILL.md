...
`selfApproval: false` / `self_approval=False` is Enterprise-only — **not available on Hallow's OSS Windmill**. Do not use.
...
- TypeScript WAC API (windmill-client): `${CLAUDE_PLUGIN_ROOT}/skills/write-workflow-as-code/references/ts-api.md`
- Python WAC API (wmill): `${CLAUDE_PLUGIN_ROOT}/skills/write-workflow-as-code/references/python-api.md`

Read the matching language reference before writing decorators / step calls — both cover: `@task`, `@workflow`, `step()`, `sleep()`, `wait_for_approval()`, `task_script`, `task_flow`, `parallel()`, error semantics.

...
- This is the OSS connect flow (stock OSS gates it behind EE/Cloud). The instance-level Google OAuth client **is configured on Hallow** (verified 2026-06-22: `oauths.gcal` / `oauths.gmail` / `oauths.google` all set; scopes incl. `calendar`, `gmail.modify`) — so the Connect button works today, no admin step needed.

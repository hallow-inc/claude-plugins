...
**Not available on this runtime.** Native TS is fetch-only — no `windmill-client` import. Use raw `fetch` to call the Windmill HTTP API directly, or switch to the `write-script-bun` skill if you need the SDK.

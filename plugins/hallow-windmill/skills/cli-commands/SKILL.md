...
Watch local file changes and live-reload the dev page for preview. Does NOT deploy to the remote workspace — at Hallow, deploy via MCP `windmill` tools (e.g. `mcp__windmill__updateScript`, `mcp__windmill__updateFlow`) or the Windmill UI. `wmill sync push` is banned at Hallow.
